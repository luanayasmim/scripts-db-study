...
create view vw_ex1 as 
	...
	from tbl_cliente c join tbl_pedido  p 
	on c.cod_cliente = p.cod_cliente
    where data_pedido > 2014-01-30;
...
create view vw_ex2 as
    ...
    from tbl_cliente c join tbl_pedido p on c.cod_cliente = p.cod_cliente
...
create view vw_ex3 as
	select p.cod_pedido, p.cod_cliente, i.qtde, i.i_cod_produto, pr.desc_produto
    from tbl_pedido p join tbl_itempedido i on p.cod_pedido = i.i_cod_pedido
...
create view vw_ex4 as
	select cod_produto, desc_produto, valor, CAST(valor * 0.112 AS DECIMAL (4,2)) reajuste, CAST(valor + (valor * 0.112) AS DECIMAL (4,2)) valor_reajuste  from tbl_produto;
    
select * from vw_ex4;
drop view vw_ex4;

select cod_produto, desc_produto, valor, CAST(valor + valor * 0.112 AS DECIMAL(2,2)) valor_reajuste  from tbl_produto;
